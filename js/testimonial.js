...
let currentItem = Math.floor(Math.random() * testimonies.length);
...
function showPerson(person) {
  const item = testimonies[person];
  img.src = item.img;
  author.textContent = item.name;
  job.textContent = item.title;
  testimony.textContent = item.description;
}

randomBtn.addEventListener("click", function () {
  currentItem = Math.floor(Math.random() * testimonies.length);
  showPerson(currentItem);
});
